...
for (let i = 1; i <= 240; i ++) {
  ...
  let offsetX = 128;
  let offsetY = 128 + (debug ? 32 : 0);

  value *= multiplier;

  let f = (x, y) => (((x - offsetX) * (x - offsetX) + (y - offsetY) * (y - offsetY)) / quotient);
  
  let vf = [0.1/256, 0.2/256, 0.3/256];//[0.001, 0.001, 0.001];//[0., 0., 1.723913];//[1.33, 0.89, 27 / 37];
  let vx = [0.1/256, 0.2/256, 0.3/256];//[0.33, 0.66, 1];
  let vy = [3/256, 2/256, 1/256];//[1.33, 0.97, 32 / 37];
